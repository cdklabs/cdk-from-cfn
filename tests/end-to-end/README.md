...
1. Create a Cloudformation stack from the input CloudFormation template to verify that the input template is valid.
2. Run cdk-from-cfn to synthesize a CDK stack in any target language from the input CloudFormation template
3. Create a working CDK App with the CDK stack definition
4. Run CDK synth on the CDK App to verify that CDK stack code generated by `cdk-from-cfn` actually works
...
6. CDK app files
   ...
   Set UPDATE_SNAPSHOTS=true run your test again. This will generate a default
