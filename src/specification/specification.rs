...
fn test_pull_json_spec() {
    ...
    let policy = specification.property_types.get("AWS::IAM::Role.Policy").unwrap();
    let policy_properties = policy.properties.as_ref().unwrap();

    assert_eq!(SimpleType::Json, policy_properties.get("PolicyDocument").unwrap().primitive_type.unwrap());
    assert_eq!(SimpleType::String, policy_properties.get("PolicyName").unwrap().primitive_type.unwrap());
}