...
type GroundStationStackProps struct {
	cdk.StackProps
...
type GroundStationStack struct {
	cdk.Stack
	SnsTopicArn interface{} // TODO: fix to appropriate type
}

func NewGroundStationStack(scope constructs.Construct, id string, props *GroundStationStackProps) *GroundStationStack {
					...
					map[string]interface{} {
						...
						"Condition": map[string]interface{} {
							...
							"ArnLike": map[string]interface{} {
								"Aws:SourceArn": jsii.String(fmt.Sprintf("arn:aws:groundstation:%v:%v:config/s3-recording/*", stack.Region(), stack.Account())),
							},
						},
					},
				},
	...
	receiverInstance := ec2.NewCfnInstance(
		...
		&ec2.CfnInstanceProps{
			...
			ImageId: amiMap[stack.Region()][jsii.String("ami")],
			...
			PolicyDocument: map[string]interface{} {
				...
				"Statement": &[]interface{}{
					map[string]interface{} {
						...
						"Resource": &[]interface{}{
							jsii.String(fmt.Sprintf("arn:aws:ec2:%v:%v:instance/%v", stack.Region(), stack.Account(), receiverInstance.Ref())),
						},
					},
					map[string]interface{} {
						"Effect": jsii.String("Allow"),
	...
	cdk.NewCfnOutput(stack, jsii.String("CfnOutputSnsTopicArn"), &cdk.CfnOutputProps{
		...
		ExportName: jsii.String(fmt.Sprintf("%v-SnsTopicArn", stack.StackName())),
		Value: snsTopic.Ref(),
	})

	return &GroundStationStack{
		Stack: stack,
