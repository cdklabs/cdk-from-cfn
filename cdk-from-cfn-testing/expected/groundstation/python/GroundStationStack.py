...
"""
class GroundStationStack(Stack):
  def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
            ...
            'Statement': [
              {
                ...
                'Condition': {
                  ...
                  'ArnLike': {
                    'aws:SourceArn': f"""arn:aws:groundstation:{self.region}:{self.account}:config/s3-recording/*""",
                  },
                },
              },
            ],
    ...
    receiverInstance = ec2.CfnInstance(self, 'ReceiverInstance',
          ...
          image_id = amiMap[self.region]['ami'],
          ...
          export HOSTNAME=$(aws ec2 describe-tags --filters "Name=resource-id,Values=$INSTANCE" "Name=key,Values=Name" --region={self.region} --output=text |cut -f5)
          ...
          aws s3 cp --region {self.region} "s3://{props['softwareS3Bucket']}/software/RT-STPS/rt-stps-process.sh" "$PROCESS_SCRIPT"
          ...
          aws sns publish --topic-arn {snsTopic.ref} --message "$MESSAGE" --region {self.region}
            ...
            'Statement': [
              {
                ...
                'Resource': [
                  f"""arn:aws:ec2:{self.region}:{self.account}:instance/{receiverInstance.ref}""",
                ],
              },
              {
                'Effect': 'Allow',
