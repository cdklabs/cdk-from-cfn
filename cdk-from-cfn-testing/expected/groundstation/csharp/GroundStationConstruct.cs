...
namespace GroundStationConstruct
{
    public class GroundStationConstructProps
    {
        /// <summary>
    ...
    /// </summary>
    public class GroundStationConstruct : Construct
    {
        public object SnsTopicArn { get; } 

        public GroundStationConstruct(Construct scope, string id, GroundStationConstructProps props = null) : base(scope, id)
        {
            // Applying default props
            props ??= new GroundStationConstructProps();
                    ...
                    { "Statement", new []
                    {
                        new Dictionary<string, object>
                        {
                            ...
                            { "Condition", new Dictionary<string, object>
                            {
                                ...
                                {
                                    { "aws:SourceArn", $"arn:aws:groundstation:{Stack.Of(this).Region}:{Stack.Of(this).Account}:config/s3-recording/*"},
                                }},
                            }},
                        },
                    }},
            ...
            var receiverInstance = new CfnInstance(this, "ReceiverInstance", new CfnInstanceProps
            {
                ...
                ImageId = amiMap[Stack.Of(this).Region]["ami"],
                ...
                export HOSTNAME=$(aws ec2 describe-tags --filters "Name=resource-id,Values=$INSTANCE" "Name=key,Values=Name" --region={Stack.Of(this).Region} --output=text |cut -f5)
                ...
                aws s3 cp --region {Stack.Of(this).Region} "s3://{props.SoftwareS3Bucket}/software/RT-STPS/rt-stps-process.sh" "$PROCESS_SCRIPT"
                ...
                aws sns publish --topic-arn {snsTopic.Ref} --message "$MESSAGE" --region {Stack.Of(this).Region}
                ...
                PolicyDocument = new Dictionary<string, object>
                {
                    ...
                    { "Statement", new []
                    {
                        new Dictionary<string, object>
                        {
                            ...
                            { "Resource", new []
                            {
                                $"arn:aws:ec2:{Stack.Of(this).Region}:{Stack.Of(this).Account}:instance/{receiverInstance.Ref}",
                            }},
                        },
                        new Dictionary<string, object>
                        {
            ...
            new CfnOutput(this, "CfnOutputSnsTopicArn", new CfnOutputProps {
                Key = "SnsTopicArn",
                ExportName = $"{Stack.Of(this).StackName}-SnsTopicArn",
