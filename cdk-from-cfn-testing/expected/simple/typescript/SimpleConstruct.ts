...
export interface SimpleConstructProps {
  /**
 ...
 */
export class SimpleConstruct extends Construct {
  /**
  ...
  public constructor(scope: Construct, id: string, props: SimpleConstructProps = {}) {
    super(scope, id);
    ...
    // Conditions
    const isUs = cdk.Fn.select(0, cdk.Stack.of(this).region.split('-')) === 'us';
    ...
    const queue = new sqs.CfnQueue(this, 'Queue', {
      ...
      queueName: [
        ...
        strings['Bars']['Bar'],
        cdk.Fn.select(1, cdk.Fn.getAzs(cdk.Stack.of(this).region)),
      ].join('-'),
