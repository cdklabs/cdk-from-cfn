...
type ConfigConstructProps struct {
...
func NewConfigConstruct(scope constructs.Construct, id string, props *ConfigConstructProps) *ConfigConstruct {
	...
	configBucket := s3.NewCfnBucket(
		...
		&s3.CfnBucketProps{
			BucketName: jsii.String(fmt.Sprintf("cdk-from-cfn-e2e-test-config-bucket-%v-%v", cdk.Stack_Of(construct).Account(), cdk.Stack_Of(construct).Region())),
		},
	)
						...
						"Statement": &[]interface{}{
							...
							map[string]interface{} {
								...
								"Resource": cdk.Fn_Join(jsii.String(""), &[]*string{
									...
									configBucket.Ref(),
									jsii.String("/AWSLogs/"),
									cdk.Stack_Of(construct).Account(),
									jsii.String("/*"),
								}),
